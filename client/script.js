...
// CODE HERE
const helloColor = () => {
    sayHelloButton.classList.add("blkAndWht")
}
...
// CODE HERE
const backToWhereYouCameFrom = () => {
    // sayHelloButton.classList.remove("blkAndWht")
    sayHelloButton.classList.add("revert")
}

sayHelloButton.addEventListener("mouseout", backToWhereYouCameFrom)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/SamRulez')
    .then((response) =>{
...
const queryTest = () => {
    axios.get('http://localhost:3000/query-test?yes')
        .then(res => {
...
const createFood = (e) => {
    e.preventDefault()
    const foodInput = document.getElementById("putni")
